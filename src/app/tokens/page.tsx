...
function BuyerEscrowRow({ escrow, onRefunded }: { escrow: EscrowEntry; onRefunded: () => void }) {
  ...
  async function handleRefund() {
    ...
    setError(null);
    try {
      await signAndExecute(buildClaimRefundTx(escrow.escrowId));
      onRefunded();
    } catch (e) {
  ...
  return (
    <tr style={{ borderBottom: "1px solid #eee" }}>
      ...
      <td style={{ padding: "0.6rem 1rem 0.6rem 0", fontSize: 13 }}>{fmtTime(escrow.redeemedAt)}</td>
      <td style={{ padding: "0.6rem 1rem 0.6rem 0", fontSize: 13 }}>{fmtTime(escrow.deliveredAt)}</td>
...
export default function TokensPage() {
  ...
  return (
    <div>
      <h1 style={{ marginBottom: "1.5rem" }}>My Tokens</h1>

      <h2 style={{ marginTop: 0, marginBottom: "0.75rem", fontSize: "1.1rem" }}>Access Tokens</h2>
      {tokensLoading && <p>Loading…</p>}
      {!tokensLoading && tokens.length === 0 && (
        <p style={{ color: "#888" }}>No access tokens yet. <a href="/">Browse services</a> to purchase one.</p>
      )}
      <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", marginBottom: "2rem" }}>
        {tokens.map((obj: any) => obj.data ? (
          <TokenCard key={obj.data.objectId} tokenObject={obj.data as Parameters<typeof TokenCard>[0]["tokenObject"]} />
        ) : null)}
      </div>
      ...
      {!keysLoading && keys.length === 0 && (
        <p style={{ color: "#888" }}>No access keys yet. Redeem an access token to receive one.</p>
      )}
      <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", marginBottom: "2rem" }}>
        {keys.map((obj: any) => obj.data ? (
          <AccessKeyCard key={obj.data.objectId} accessKeyObject={obj.data as Parameters<typeof AccessKeyCard>[0]["accessKeyObject"]} />
        ) : null)}
      </div>
